...
def get_scatter_chart(entered_site, selected_range):
    ...
    if entered_site == 'ALL':
        ...
        title = f'Scatter plot for All Sites - Payload Range: {selected_range[0]} to {selected_range[1]} kg'
    else:
        filtered_df = spacex_df[(spacex_df['Launch Site'] == entered_site) &
                                (spacex_df['Payload Mass (kg)'] >= selected_range[0]) &
                                (spacex_df['Payload Mass (kg)'] <= selected_range[1])]
        title = f'Scatter plot for {entered_site} - Payload Range: {selected_range[0]} to {selected_range[1]} kg'

    # Create the scatter plot
    fig = px.scatter(filtered_df, 
                     x='Payload Mass (kg)', 
                     y='class', 
                     color='Booster Version Category',  # Color-label points by Booster Version Category
                     title=title,
